        ...
        <table class="table table-hover table-dark mb-0 text-center">
          ...
          @foreach($staff as $stu)
              <tr>
                ...
                <td>{{$stu -> cell}}</td>
                <td>{{$stu -> uname}}</td>
                <td><img src="{{URL::to('/')}}/media/staff/{{$stu -> photo}}" alt="" style="width: 50px;height: 50px;"></td>
                <td>{{$stu -> age}}</td>
                <td>{{$stu -> created_at -> diffForHumans()}}</td>
                <td class="text-right">
                    <div class="btn-group">
                        <a href="{{route('staff.show', $stu -> id)}}" class="btn btn-info rounded-0">View</a>
                        <a href="{{route('staff.edit', $stu -> id)}}" class="btn btn-warning rounded-0">Edit</a>
                        <form action="{{route('staff.destroy', $stu -> id)}}" method="POST">
                            @csrf
                            @method('DELETE')
